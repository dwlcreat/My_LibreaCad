...
struct RS_ActionEeResistance::ImageData {
	RS_ImageData data;
	QImage img;
};
 ...
 * Constructor.
 */
RS_ActionEeResistance::RS_ActionEeResistance(RS_EntityContainer& container,
...
RS_ActionEeResistance::~RS_ActionEeResistance() = default;


void RS_ActionEeResistance::init(int status) {
    RS_PreviewActionInterface::init(status);

    reset();

    //pImg->data.file = RS_DIALOGFACTORY->requestImageOpenDialog();
    pImg->data.file  = RESISTANCE_PATH;
...
void RS_ActionEeResistance::reset() {
...
void RS_ActionEeResistance::trigger() {
    deletePreview();
	if (!pImg->data.file.isEmpty()) {
        RS_Creation creation(container, graphicView);
		creation.createImage(& pImg->data);
    }
    pImg->data.uVector=PRIVATE_U_VECTOR;
    pImg->data.vVector=PRIVATE_V_VECTOR;
...
void RS_ActionEeResistance::mouseMoveEvent(QMouseEvent* e) {
...
void RS_ActionEeResistance::mouseReleaseEvent(QMouseEvent* e) {
...
void RS_ActionEeResistance::coordinateEvent(RS_CoordinateEvent* e) {
	if (e==nullptr) {
        return;
    }
...
void RS_ActionEeResistance::commandEvent(RS_CommandEvent* e) {
...
double RS_ActionEeResistance::getAngle() const {
	return pImg->data.uVector.angle();
}

void RS_ActionEeResistance::setAngle(double a) const{
	double l = pImg->data.uVector.magnitude();
	pImg->data.uVector.setPolar(l, a);
	pImg->data.vVector.setPolar(l, a+M_PI_2);
}

double RS_ActionEeResistance::getFactor() const {
	return pImg->data.uVector.magnitude();
}

void RS_ActionEeResistance::setFactor(double f) const {
	double a = pImg->data.uVector.angle();
	pImg->data.uVector.setPolar(f, a);
	pImg->data.vVector.setPolar(f, a+M_PI_2);
}

double RS_ActionEeResistance::dpiToScale(double dpi) const {
	return RS_Units::dpiToScale(dpi, document->getGraphicUnit());
}



double RS_ActionEeResistance::scaleToDpi(double scale) const {
	return RS_Units::scaleToDpi(scale, document->getGraphicUnit());
}



void RS_ActionEeResistance::updateMouseCursor() {
	graphicView->setMouseCursor(RS2::CadCursor);
}

QStringList RS_ActionEeResistance::getAvailableCommands() {
    QStringList cmd;
...
void RS_ActionEeResistance::showOptions() {
...
void RS_ActionEeResistance::hideOptions() {
    RS_ActionInterface::hideOptions();
	RS_DIALOGFACTORY->requestOptions(this, false);
}


void RS_ActionEeResistance::updateMouseButtonHints() {
    switch (getStatus()) {
    case SetTargetPoint:
